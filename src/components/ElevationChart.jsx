      ...
      <ResponsiveContainer width="100%" height="100%" className="elevation-chart">
        ...
        >
          ...
          <XAxis dataKey="dist" 
            ...
            tickMargin={3}
            label={{value: "Distance (km)", position: "insideBottom", offset: -15, "fill": "var(--_clr-chart-black)"}}
          />
          <YAxis
            type="number" // unit="m"
            ...
            label={{value: "Ele (m)", angle: -90, position: "center", "fill": "var(--_clr-chart-black)"}}
          />
...
const ActiveDot = ({ payload, setActiveIndex }) => {
  ...
  return (
    <>
      <text fill="var(--_clr-chart-black)" textAnchor={atStart ? "start" : (atEnd ? "end" : "middle")} x={atStart ? 65-23 : (atEnd ? svgWidth-5 : payload.cx)} y={20} style={{fontSize: "var(--fs-300)", fontWeight: "var(--fw-medium)"}}>{payload?.payload?.dist?.toFixed(1) + "km"}</text>
      <text fill="var(--_clr-chart-black)" textAnchor={atEndLong ? "end" : "start"} x={atEndLong ? payload.cx - 10 : payload.cx + 10} y={atBottom ? svgHeight - 55 : payload.cy + 5} style={{fontSize: "var(--fs-300)", fontWeight: "var(--fw-medium)", textShadow: "white -0.125rem 0, white 0.125rem 0, white 0 -0.125rem, white 0 0.125rem"}}>{payload?.payload?.ele + "m"}</text>
      <svg x={payload.cx - halfwidth} y={payload.cy - halfwidth} width={width} height={width} fill="var(--_clr-chart-stroke)" stroke="var(--_clr-chart-black)" viewBox="0 0 24 24" strokeWidth={5}>
...
const CustomWaypointDot = ({ cx, cy, payload }) => {
  ...
  if (payload.waypoint?.length > 0) {
    return (
      <svg x={cx - halfwidth + 1} y={cy - width} width={width} height={width} viewBox="0 0 24 24" fill="var(--_clr-chart-stroke)" stroke="var(--_clr-chart-black)" strokeWidth={1}>
