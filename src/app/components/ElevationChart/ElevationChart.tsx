...
export default function ElevationChart({ data, setActiveIndex, showHillMarkers=false } : { data: ElevationPoint[]; setActiveIndex?: CallableFunction; showHillMarkers?: boolean }) {
  ...
  return (
    data && 
      <ResponsiveContainer width="100%" height="100%" className={styles.elevationChart}>
        <AreaChart data={data}
          id="elevation-chart"
        ...
        >
          ...
          <ReferenceLine
            segment={[{ x: 0, y: 0 }, { x: 0, y: data[0].ele }]}
            stroke="var(--_clr-chart-black-light)"
            strokeWidth={1}
          />
          {showHillMarkers && waypoints?.map((wp, index) => {
            return (
              <ReferenceLine
                key={index}
                segment={[{ x: wp.dist, y: 0 }, { x: wp.dist, y: wp.ele }]}
                stroke="var(--_clr-chart-black-light)"
                strokeWidth={1}
              />
            )
          ...
          <ReferenceLine
            segment={[{ x: totalDist, y: 0 }, { x: totalDist, y: data[data.length-1].ele }]}
            stroke="var(--_clr-chart-black-light)"
            strokeWidth={1}
          />
...
const CustomToolTipContent = ({ payload, setActiveIndex } : { payload?: TooltipPayloadItem[]; setActiveIndex: CallableFunction }) => {
...
const ActiveDot = ({ payload, setActiveIndex } : { payload: ActiveDotProps; setActiveIndex: CallableFunction }) => {
  ...
  const svgElement = document.getElementById("elevation-chart") as SVGSVGElement | null;
...
const CustomWaypointDot = ({ cx=0, cy=0, payload } : { cx?: number; cy?: number; payload?: ElevationPoint}) => {
