...
export default function EstimatedTime({
  ...
  selected: boolean;
  walkLengthInKm: number | undefined;
}) {
  ...
  return (
    ...
    >
      ...
      <p aria-live="polite">
        An average hiking pace of <b>{displayDistance(speedInKm, 1) + "/h"}</b>{" "}
        completes this walk in <b>{timeTaken}</b>
      </p>
      <input
        type="range"
        min={1}
        ...
        step={0.1}
        value={speedInKm}
        onChange={(e) => setSpeedInKm(Number(e.target.value))}
